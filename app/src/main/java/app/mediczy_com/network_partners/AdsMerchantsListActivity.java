...
public class AdsMerchantsListActivity extends BaseActivity implements ResponseListener {

    private Toolbar toolbar;
    private RecyclerView mRecyclerView;
    private RecyclerView.LayoutManager mLayoutManager;
    private ProgressDialog pDialog = null;
    private Ads_MerchantsAdapter adapter_list;
    private RequestCommon requestCommon;
    private Bundle bundle;
    private int mRequestType = 0;
    public String Id, latitude="", longitude="", name="", type="";

    ArrayList<Home_Type_Bean> arrayList = new ArrayList<Home_Type_Bean>();
    ...
    private void onRequest() {
        mRequestType = IConstant_WebService.WSR_NP_NetworkPartnerAds_List_Req;
        if (Utility.getInstance().isConnectingToInternet(this)) {
            ...
            RequestManager.NetworkPartnersAdsHome(this, null, requestCommon, mRequestType);
        } else {
    ...
    public void onResponseReceived(Object responseObj, int requestType) {
        ...
        if (requestType == IConstant_WebService.WSR_NP_NetworkPartnerAds_List_Req) {
            CommonResponse res = (CommonResponse) responseObj;
            MLog.e("Ads HospitalListActivity :", "" + res);
            if (res != null) {
    ...
    private void setData(CommonResponse res) {
        if (res.adList != null) {
            if (res.adList.size() >0) {
                for (int i = 0; i < res.adList.size(); i++) {
                    Home_Type_Bean bean = new Home_Type_Bean();
                    bean.setTitleAds(res.adList.get(i).notification_title);
                    bean.setDescAds(res.adList.get(i).notification_msg);
                    bean.setDateAds(res.adList.get(i).notification_time);
                    bean.setImageAds(res.adList.get(i).notification_image);
                    bean.setTypeAds(res.adList.get(i).type);
                    bean.setCompanyLogo(res.adList.get(i).company_image);
                    bean.setCompanyName(res.adList.get(i).company_name);
                    bean.setViewCount(res.adList.get(i).company_view_count);
                    bean.setIdAds(res.adList.get(i).ads_id);
                    bean.setType(Home_Type_Bean.TYPE_ADS);
                    bean.setView_type(res.adList.get(i).view_type);
                    arrayList.add(bean);
                }
                adapter_list = new Ads_MerchantsAdapter(AdsMerchantsListActivity.this, arrayList);
                mRecyclerView.setAdapter(adapter_list);
                Utility.getInstance().runLayoutAnimation(mRecyclerView, this);
            } else {
                Utility.getInstance().showToast("Ads List is Empty", this);
            }
        }
    }
}
