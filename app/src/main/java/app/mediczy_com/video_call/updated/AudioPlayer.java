...
public class AudioPlayer {
            ...
            case AudioManager.RINGER_MODE_NORMAL:
                ...
                try {
                    mPlayer.setDataSource(mContext,
                            Uri.parse("android.resource://" + mContext.getPackageName() + "/" + R.raw.video_call_ringtone));
                    mPlayer.prepare();
                } catch (IOException e) {
                    MLog.e(LOG_TAG, "Could not setup media player for ringtone");
                    Utility.getInstance().showToast("Could not setup media player for ringtone", mContext);
                    mPlayer = null;
                    return;
                }
    ...
    public void playProgressTone() {
        ...
        try {
            ...
            MLog.e(LOG_TAG, "Could not play progress tone", e);
        }
    }
